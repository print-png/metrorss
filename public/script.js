...
async function loadNews(silent) {
    ...
    try {
        ...
        if (!res.ok) {
            const errData = await res.json().catch(() => ({}));
            if (res.status === 403 && errData.error) {
                feed.innerHTML = '<div class="post" style="border-color:#e51400;"><p style="color:#e51400;font-weight:300;"><b>вы забанены</b>' + (errData.reason ? '<br><span style="color:#999;font-size:13px;">причина: ' + esc(errData.reason) + '</span>' : '') + (errData.deviceId ? '<br><span style="color:#555;font-size:11px;">device: ' + esc(errData.deviceId) + ' <button class="copy-btn" onclick="var d=\'' + esc(errData.deviceId) + '\',ok=!1,t=document.createElement(\'textarea\');t.value=d;document.body.appendChild(t);t.select();try{ok=document.execCommand(\'copy\')}catch(e){}t.remove();if(ok){(function(){var to=document.getElementById(\'toast\');if(!to){to=document.createElement(\'div\');to.id=\'toast\';to.style.cssText=\'position:fixed;bottom:60px;left:50%;transform:translateX(-50%);background:#744da9;color:#fff;padding:10px 24px;font-size:14px;border-radius:4px;opacity:0;z-index:999;pointer-events:none;transition:opacity 0.25s ease;\';document.body.appendChild(to)}to.textContent=\'скопировано\';to.style.opacity=\'1\';clearTimeout(to._t);to._t=setTimeout(function(){to.style.opacity=\'0\'},2000)})()}else{prompt(\'Скопируй вручную (Ctrl+C):\',d)}">📋 копировать</button></span>' : '') + '</p></div>';
                return;
            }
